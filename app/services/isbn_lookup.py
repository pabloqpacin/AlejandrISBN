...
async def lookup_isbn(isbn: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT, headers=headers, follow_redirects=True) as client:
        # Prefer Spanish catalog for local ISBNs; race OL at the same time.
        primary = await _first_hit(
            [
                *(_todos_tus_libros(client, v) for v in plain_variants[:2]),
    ...
    if not merged.get("title"):
        # Last resort: sequential deeper TTL + OL search over all variants
        async with httpx.AsyncClient(timeout=TIMEOUT, headers=headers, follow_redirects=True) as client:
            for variant in variants:
                for fetcher in (_todos_tus_libros, _open_library_search, _google_books):
